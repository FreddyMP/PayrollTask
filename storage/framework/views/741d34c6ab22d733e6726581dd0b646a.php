...
<div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <div class="d-flex align-items-center gap-2">
        ...
        <div class="d-flex gap-1 me-2 pe-2 border-end border-white-10">
            <button class="rest-config-btn <?php echo e($company->saturday_rest ? 'active' : ''); ?>" onclick="toggleRest('sat')" title="Sábados como descanso">
                <i class="bi bi-calendar-week"></i> Sáb <span class="d-none d-md-inline">Descanso</span>
            </button>
            <button class="rest-config-btn <?php echo e($company->sunday_rest ? 'active' : ''); ?>" onclick="toggleRest('sun')" title="Domingos como descanso">
        ...
        <?php endif; ?>
        <a href="<?php echo e(route('calendar.create')); ?>" class="btn btn-primary-custom">
...
<script>
    ...
    let satRest = <?php echo e($company->saturday_rest ? 'true' : 'false'); ?>;
    let sunRest = <?php echo e($company->sunday_rest ? 'true' : 'false'); ?>;
    const isSpecialUser = <?php echo e((auth()->user()->role === 'super' || auth()->user()->role === 'admin') ? 'true' : 'false'); ?>;
    ...
    function loadCalendar() {
        ...
        const eventsReq = fetch(`<?php echo e(route('calendar.apiEvents')); ?>?year=${currentYear}&month=${currentMonth + 1}&view=${currentView}`).then(r => r.json());
        const holidaysReq = fetch(`<?php echo e(route('calendar.apiHolidays')); ?>?year=${currentYear}&month=${currentMonth + 1}`).then(r => r.json());
    ...
    function toggleRest(type) {
        fetch(`<?php echo e(route('calendar.toggleWeekendRest')); ?>`, {
            method: 'POST',
            headers: {
    ...
    function toggleHoliday(date, name) {
        fetch(`<?php echo e(route('calendar.toggleHoliday')); ?>`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
    ...
    function dayClick(day, dateStr) {
        ...
        if (dayEvents.length > 0) {
            dayEvents.forEach(e => {
                ...
                if (e.is_owner) {
                    ...
                    html += `<?php echo csrf_field(); ?> <?php echo method_field('DELETE'); ?>`;
        ...
        html += `<a href="<?php echo e(route('calendar.create')); ?>?date=${dateStr}" class="add-event-btn mt-2"><i class="bi bi-plus-circle"></i> Agregar actividad</a>`;
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Freddy\Desktop\proyectos\anti\resources\views/calendar/index.blade.php ENDPATH**/ ?>