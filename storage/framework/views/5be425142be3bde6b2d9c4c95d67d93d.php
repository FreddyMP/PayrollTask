<?php $__env->startSection('title', 'Bonificaciones de Ley'); ?>
<?php $__env->startSection('page-title', 'Bonificaciones de Ley (Utilidades)'); ?>

<?php $__env->startSection('content'); ?>

    <?php if(session('payroll_exists_error')): ?>
        <!-- Modal de Error de Nómina Existente -->
        <div class="modal fade" id="payrollExistsModal" tabindex="-1" aria-hidden="true" data-bs-backdrop="static"
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <p class="text-white mb-3">
                            La <strong>nómina del período actual ya ha sido generada</strong>. Para agregar bonificaciones de
                            ley a la nómina, esta <strong>NO debe estar generada previamente</strong>.
                        </p>
                            ...
                            <div class="card-body">
                                ...
                                <ol class="text-white small mb-0">
                                    <li class="mb-2">
                                        ...
                                        <br>
                                        <small class="text-muted">Ve a "Registros de Nómina" y elimina las nóminas del período
                                            <?php echo e(now()->format('Y-m')); ?> que aún no estén marcadas como pagadas.</small>
                                    </li>
                                    <li class="mb-2">
                                        <strong>Agregar bonificaciones manualmente</strong>
                                        <br>
                                        <small class="text-muted">Edita cada nómina y agrega el monto en el campo "Extras" o
                                            "Bonificaciones".</small>
                                    </li>
                                    <li>
                                        <strong>Usar pago separado</strong>
                                        <br>
                                        <small class="text-muted">Cambia la configuración en "Empresa" para pagar bonificaciones
                                            de forma separada.</small>
                                    </li>
                                </ol>
                            </div>
                        </div>
                        ...
                        <p class="text-muted small mb-0">
                            ...
                            <strong>Nota:</strong> Las bonificaciones deben agregarse ANTES de generar la nómina del mes para
                            que se calculen correctamente los impuestos y deducciones.
                        </p>
                    </div>
    ...
    <ul class="nav nav-tabs mb-4 px-3" style="border-bottom: 1px solid rgba(255,255,255,0.1);">
        ...
        <li class="nav-item">
            <a class="nav-link active" href="<?php echo e(route('payroll.bonuses')); ?>"
    ...
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center text-white">
            ...
            <div>
                <span class="badge badge-status badge-info me-2" id="employeeCount"><?php echo e(count($employees)); ?> Empleados</span>
                <?php if(Auth::user()->company->bonus_payment_method === 'payroll'): ?>
                    <form id="addBonusesForm" action="<?php echo e(route('payroll.bonuses.addToPayroll')); ?>" method="POST"
                        class="d-inline">
                        <?php echo csrf_field(); ?>
                ...
                <table class="table mb-0" id="bonusesTable">
                    <thead>
                        <tr>
                            ...
                            <?php if(Auth::user()->company->bonus_payment_method === 'separate'): ?>
                    ...
                    <tbody>
                        <?php $__currentLoopData = $employees; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $employee): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            ...
                            ?>
                            <tr class="employee-row" data-salary="<?php echo e($employee->salary); ?>" data-years="<?php echo e($years); ?>"
                                data-base-bonus="<?php echo e($baseBonus); ?>">
                                <td class="fw-semibold"><?php echo e($employee->user->name ?? '—'); ?></td>
                                <td><?php echo e($hireDate?->format('d/m/Y') ?? '—'); ?></td>
                                <td><?php echo e($years); ?> años</td>
                                <td>RD$ <?php echo e(number_format($employee->salary, 2)); ?></td>
                                <td>RD$ <?php echo e(number_format($valorDia, 2)); ?></td>
                                <td><?php echo e($dias); ?></td>
                                <td>RD$ <?php echo e(number_format($baseBonus, 2)); ?></td>
                                <td class="text-end fw-bold text-success final-bonus-cell"
                                    data-employee-id="<?php echo e($employee->id); ?>">RD$ 0.00</td>
                                <?php if(Auth::user()->company->bonus_payment_method === 'separate'): ?>
                                    <td class="text-center">
                                        <form action="<?php echo e(route('payroll.bonuses.paySeparate', $employee)); ?>" method="POST"
                                            class="d-inline separate-pay-form"
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Freddy\Desktop\proyectos\anti\resources\views/payroll/bonuses.blade.php ENDPATH**/ ?>