    ...
    <div class="container">
        ...
        <div class="content">
            ...
            <div class="field">
                ...
                <div class="value" style="font-weight: 600; font-size: 18px;">{{ $incident->title }}</div>
            </div>

            <div class="field">
                <div class="label">Reportado por</div>
                <div class="value">{{ $incident->user->name ?? 'Usuario Desconocido' }} ({{ $incident->user->email ?? 'Sin Correo' }})</div>
            </div>
            ...
            <div class="field">
                ...
                <div class="value" style="background-color: #f8fafc; padding: 12px; border-radius: 8px; border: 1px solid #e2e8f0; white-space: pre-line;">{{ $incident->description }}</div>
            </div>

            @if($incident->attachments && $incident->attachments->isNotEmpty())
                        ...
                        <ul style="padding-left: 20px; margin: 5px 0; list-style-type: none;">
                            @foreach($incident->attachments as $attachment)
                                <li style="margin-bottom: 8px;">
                                    <a href="{{ \Illuminate\Support\Facades\Storage::disk('s3')->url($attachment->file_path) }}" target="_blank" style="color: #6366f1; text-decoration: none; font-weight: 600; font-size: 14px; display: inline-flex; align-items: center; gap: 6px;">
