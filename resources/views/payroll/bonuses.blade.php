...
@section('content')
                                ...
                                <ol class="text-white small mb-0">
                                    <li class="mb-2">
                                        ...
                                        <small class="text-muted">Ve a "Registros de Nómina" y elimina las nóminas del período
                                            {{ now()->format('Y-m') }} que aún no estén marcadas como pagadas.</small>
                                    </li>
                                    <li class="mb-2">
    ...
    <ul class="nav nav-tabs mb-4 px-3" style="border-bottom: 1px solid rgba(255,255,255,0.1);">
        ...
        <li class="nav-item">
            <a class="nav-link active" href="{{ route('payroll.bonuses') }}"
        ...
        <div class="card-header d-flex justify-content-between align-items-center text-white">
            ...
            <div>
                ...
                @if(Auth::user()->company->bonus_payment_method === 'payroll')
                    <form id="addBonusesForm" action="{{ route('payroll.bonuses.addToPayroll') }}" method="POST"
                        class="d-inline">
                        @csrf
                ...
                <table class="table mb-0" id="bonusesTable">
                    ...
                    <tbody>
                        @foreach($employees as $employee)
                            ...
                            @endphp
                            <tr class="employee-row" data-salary="{{ $employee->salary }}" data-years="{{ $years }}"
                                ...
                                <td class="text-end fw-bold text-success final-bonus-cell"
                                    data-employee-id="{{ $employee->id }}">RD$ 0.00</td>
                                @if(Auth::user()->company->bonus_payment_method === 'separate')
                                    <td class="text-center">
                                        <form action="{{ route('payroll.bonuses.paySeparate', $employee) }}" method="POST"
