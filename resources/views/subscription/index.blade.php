...
<body>
    ...
    @if(auth()->user()->isSuper())
        {{-- Super user sees the plan selection --}}
        <div class="hero">
            <div class="badge-trial">⚠️ Período de prueba expirado</div>
            <h1>Elige tu <span>Plan de Suscripción</span></h1>
            <p>{{ $company->name }} — Selecciona el plan que mejor se adapte a tu empresa para continuar usando la plataforma.</p>
        </div>
        ...
        <div class="plans-grid">
            @foreach($plans as $key => $plan)
                <div class="plan-card {{ isset($plan['popular']) && $plan['popular'] ? 'popular' : '' }}">
                    @if(isset($plan['popular']) && $plan['popular'])
                        <div class="popular-badge">⭐ Más Popular</div>
                    @endif

                    <div class="plan-icon" style="background: linear-gradient(135deg, {{ $plan['color'] }}, {{ $plan['color'] }}aa);">
                        <i class="bi {{ $plan['icon'] }}"></i>
                    </div>

                    <div class="plan-name">{{ $plan['name'] }}</div>
                    <div class="plan-limit"><i class="bi bi-people-fill me-1"></i>{{ $plan['limit'] }}</div>
                    ...
                    <form method="POST" action="{{ route('subscription.store') }}">
                        ...
                        <button type="submit" class="select-btn" style="background: linear-gradient(135deg, {{ $plan['color'] }}, {{ $plan['color'] }}cc);">
    ...
    @else
        {{-- Non-super user sees the locked message --}}
        <div class="locked-container">
            ...
            <p>
                El período de prueba gratuita de <strong>{{ $company->name }}</strong> ha expirado.<br><br>
