                ...
                <div class="card ranking-top-{{ $loop->iteration }} h-100">
                    <div class="card-body text-center py-4">
                        <div class="d-flex justify-content-center mb-3">
                            <div
                                class="trophy-badge bg-{{ $loop->iteration === 1 ? 'warning' : ($loop->iteration === 2 ? 'secondary' : 'danger') }} text-white">
                        ...
                        @forelse($candidates as $candidate)
                            <tr>
                                <td class="text-center">
                                    @if($loop->iteration <= 3)
                                        <i
                                            class="bi bi-star-fill text-{{ $loop->iteration === 1 ? 'warning' : ($loop->iteration === 2 ? 'secondary' : 'danger') }}"></i>
                                    @else
                                ...
                                <td>
                                    <div class="d-flex gap-1 flex-wrap">
                                        @foreach($vacancy->steps as $step)
                                            ...
                                            @endphp
                                            <div class="badge bg-dark" style="font-size: 0.65rem;">
                                ...
                                <td class="text-end">
                                    <span class="text-dark fw-bold h5 mb-0">{{ $candidate->total_points }}</span>
                                    <small class="text-dark px-1">/ {{ $vacancy->steps->sum('points') }}</small>
                                </td>
                            <td class="text-end">

                                                                       @if($candidate->status === 'hired')
                                                                                            <span class="badge bg-success"><i class="bi bi-check-circle-fill me-1"></i>Seleccio
                                                                        n                       ado</span>
