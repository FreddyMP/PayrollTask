...
@section('content')
        ...
        <div class="col-md-3">
            <div class="stat-card green">
                ...
                <div class="stat-value">
                    {{ $payrollSummaryLatest ? number_format($payrollSummaryLatest->total_net, 0, '.', ',') : '0' }}</div>
                            ...
                            <tbody>
                                @forelse($payrollHistory as $ph)
                                    <tr>
                                        ...
                                        <td style="color:#34d399;font-weight:600;">RD$ {{ number_format($ph->total_net, 2) }}
                                        </td>
                                        <td>
                            ...
                            @if($deductionBreakdown)
                                ...
                                @foreach($dItems as $d)
                                    <div class="d-flex align-items-center gap-2 mb-2">
                                        <div
                                            style="width:10px;height:10px;border-radius:3px;background:{{ $d['color'] }};flex-shrink:0;">
                                        </div>
                                        ...
                                        <div
                                            ...
                                            <div
                                                style="height:100%;width:{{ ($d['val'] / $dTotal) * 100 }}%;background:{{ $d['color'] }};border-radius:4px;transition:width 0.6s ease;">
                                            </div>
                                        </div>
                                        <span
                                            style="font-size:0.78rem;font-weight:600;color:{{ $d['color'] }};min-width:75px;text-align:right;">RD$
                                            {{ number_format($d['val'], 0) }}</span>
                                    </div>
                                @endforeach
                            @else
                        ...
                        <div class="dash-pipeline-item">
                            <div class="d-flex justify-content-between align-items-start mb-2">
                                <div>
                                    ...
                                    <div style="font-size:0.72rem;color:#64748b;">{{ $v->department ?: 'Sin depto.' }} ·
                                        {{ $v->steps_count }} pasos</div>
                                </div>
                                <div class="d-flex gap-1">
                                    <span class="badge-status badge-active" style="font-size:0.6rem;">{{ $v->candidates_count }}
                                        cand.</span>
                                    @if($v->hired_count > 0)
                                        <span class="badge-status badge-completed" style="font-size:0.6rem;">{{ $v->hired_count }}
                                            contrat.</span>
                                    @endif
                                    @if($v->discarded_count > 0)
                                        <span class="badge-status badge-rejected"
                                            style="font-size:0.6rem;">{{ $v->discarded_count }} desc.</span>
                                    @endif
                                </div>
                            </div>
                            @php
        ...
        <div class="col-lg-4">
            ...
            <div class="card mb-3">
                ...
                <div class="card-body py-2 px-3 text-white">
                    ...
                    @forelse($requestsByType as $type => $statuses)
                        <div class="d-flex align-items-center gap-2 py-2"
                            ...
                            <div
                                ...
                                <i class="bi {{ $typeIcons[$type] ?? 'bi-question-circle' }}"
                                    style="color:{{ $typeColors[$type] ?? '#94a3b8' }};font-size:0.85rem;"></i>
                            </div>
                            <div style="flex:1;">
                    ...
                    @endforelse
                    <a href="{{ route('requests.index') }}" class="btn btn-outline-custom btn-sm w-100 mt-2">Ver
                        solicitudes</a>
                </div>
            </div>
                    ...
                    @forelse($departmentDistribution as $dept)
                        <div class="d-flex align-items-center gap-2 mb-2">
                            <span
                                ...
                                title="{{ $dept->department }}">{{ $dept->department }}</span>
                            <div style="flex:1;height:8px;background:rgba(255,255,255,0.06);border-radius:4px;overflow:hidden;">
                                <div
                                    style="height:100%;width:{{ ($dept->count / $maxDept) * 100 }}%;background:var(--gradient-1);border-radius:4px;">
                                </div>
                            </div>
                            <span
                                style="font-size:0.78rem;font-weight:700;color:white;min-width:20px;text-align:right;">{{ $dept->count }}</span>
                        </div>
                    @empty
                    ...
                    @forelse($recentAccess as $log)
                        <div class="d-flex align-items-center gap-3 px-3 py-2"
                            ...
                            <div>
                                ...
                                <div style="font-size:0.68rem;color:#64748b;">{{ $log->login_at->format('d/m H:i') }} —
                                    {{ $log->logout_at ? $log->logout_at->format('H:i') : 'Activo' }}</div>
                            </div>
                        </div>
                    @empty
    ...
    {{-- PAYROLL CONFIGURATION MODAL (obligatorio para super sin configurar) --}}
            ...
            <div class="pf-modal">
                ...
                <div class="pf-modal-body">
                    <p class="pf-desc">
                        ...
                        <strong style="color:white;">{{ Auth::user()->company->name }}</strong>.
                    </p>
        ...
        <div class="today-modal-overlay" id="todayModal">
            <div class="today-modal">
                <div class="today-modal-header">
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <div>
                            ...
                            <small
                                style="color:var(--dark-4);font-size:0.7rem;">{{ now()->translatedFormat('l, d \\d\\e F Y') }}</small>
                        </div>
                    </div>
                    ...
                    @foreach($todayEvents as $event)
                        <div class="today-event-card">
                            <div class="today-event-time"><i
                                    class="bi bi-clock me-1"></i>{{ \Carbon\Carbon::parse($event->event_time)->format('h:i A') }}
                            </div>
                            <div class="today-event-title">{{ $event->title }}</div>
                            @if($event->description)
                            <div class="today-event-desc">{{ $event->description }}</div>@endif
                            @if($event->links->isNotEmpty())
                                <div class="today-event-links">
                                    @foreach($event->links as $link)
                                        <a href="{{ $link->url }}" target="_blank"><i
                                                class="bi bi-link-45deg"></i>{{ $link->label ?: $link->url }}</a>
                                    @endforeach
                                </div>
                            @endif
                        </div>
                ...
                <div class="today-modal-footer">
                    <a href="{{ route('calendar.index') }}" class="btn btn-primary-custom w-100"><i
